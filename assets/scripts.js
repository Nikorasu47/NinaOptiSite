document.addEventListener("DOMContentLoaded", function () {
  ...
  const tags = Array.from(
    new Set(images.map((img) => img.dataset.galleryTag).filter(Boolean))
  );
  if (tags.length) {
    ...
    ul.addEventListener("click", (e) => {
      ...
      ul.querySelectorAll(".active, .active-tag").forEach((el) =>
  ...
  function getVisibleImages() {
    return Array.from(gallery.querySelectorAll(".gallery-item")).filter(
      (img) => {
        const col = img.closest(".item-column");
